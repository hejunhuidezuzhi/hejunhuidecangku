...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/bill/providerlist.html",//请求的url
		...
		success:function(data){//data：返回数据（json对象）
			...
			if(msg != null){
				...
				for(var i = 0; i < msg.length; i++){
					...
					if(pid != null && pid != undefined && msg[i].id == pid ){
						options += "<option selected=\"selected\" value=\""+msg[i].id+"\" >"+msg[i].proName+"</option>";
					}else{
						options += "<option value=\""+msg[i].id+"\" >"+msg[i].proName+"</option>";
					}
