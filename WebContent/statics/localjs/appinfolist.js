$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"../devuser/categorylevellist.json",//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="../devuser/appversionadd.html?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == "" || versionid=="0"){
			...
			window.location.href="../devuser/appversionmodify.html?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="../devuser/appinfomodify.html?id="+ obj.attr("appinfoid");
	}else{
...
var saleSwitchAjax = function(appId,obj){
	alert(obj.attr("saleSwitch"));
	var sale=obj.attr("saleSwitch");
	$.ajax({
		type:"get",
		url:"../devuser/sale.json",
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href=path+"/devuser/appview/"+ obj.attr("appinfoid");
});
